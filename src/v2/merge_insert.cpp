...
namespace diskann {
  ...
  MergeInsert<T, TagT>::MergeInsert(
    ...
    _disk_index = new diskann::PQFlashIndex<T, TagT>(this->_dist_metric, reader,
  ...
  void MergeInsert<T, TagT>::final_merge(uint32_t id_map) {
    ...
    diskann::cout << _mem_index_0->get_num_points() << "  "

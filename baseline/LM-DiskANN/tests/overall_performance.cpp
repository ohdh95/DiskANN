...
#pragma omp parallel for num_threads(NUM_SEARCH_THREADS)
  ...
  if (calRecall) {
    ...
    // // 逐行读取文件内容
    // while (std::getline(reader_tags, line)) {
    //   std::stringstream ss(line);
    //   int               tags, rm_tags;
    //   ss >> tags >> rm_tags;
    //   if (tags < remove_tags.size()) {
    //     remove_tags[tags] = rm_tags;
    //     // std::cout << tags << " " << rm_tags << std::endl;
    //   } else {
    //     std::cout << tags << " > " << remove_tags.size() << std::endl;
    //   }
    // }
    // std::cout << "Read Over\n";
    // // std::vector<unsigned> rm_gt_tags(gt_num * gt_dim);
    // unsigned* rm_gt_tags = new unsigned[gt_num * gt_dim];
    // for (size_t i = 0; i < gt_num; i++) {
    //   for (size_t j = 0; j < gt_dim; j++) {
    //     // std::cout << gt_tags[i * gt_dim + j] << " ";

    //     rm_gt_tags[i * gt_dim + j] = remove_tags[gt_ids[i * gt_dim + j]];
    //     std::cout << gt_ids[i * gt_dim + j] << "-"
    //               << remove_tags[gt_ids[i * gt_dim + j]] << " ";
    //   }
    //   std::cout << std::endl;
    // }
    // for (size_t i = 0; i < gt_num; i++) {
    //   for (size_t j = 0; j < gt_dim; j++) {
    //     std::cout << gt_tags[i * gt_dim + j] << "-" << gt_ids[i * gt_dim + j]
    //               << " ";
    ...
    //   uint32_t* node_id = query_result_tags + i * gt_dim;
    //   std::cout << "#query_id " << i << " 's res:";
    //   for (uint32_t j = 0; j < gt_dim; j++)
    //     std::cout << " " << node_id[j];
    //   std::cout << std::endl;
    // }
    // 计算recall
  ...
  std::cout << "search current time: " << diff_search.count() << std::endl;
...
void deletion_kernel(T* data_load, diskann::MergeInsert<T, TagT>& sync_index,
                     std::vector<TagT>& delete_vec, size_t aligned_dim, _u64 L,
                     int md = -1) {
  diskann::Timer      timer;
...
void update(const std::string& data_path, const unsigned L_mem,
  ...
  // diskann::load_aligned_bin<T>(data_path.c_str(), data_load,
  // num_points, dim,
  //                              aligned_dim);
  ...
  diskann::MergeInsert<T, TagT> sync_index(
      ...
      metric, false, save_path, start_step);
  ...
  for (int i = 0; i < batch; i++) {
    ...
    deletion_kernel(data_load, sync_index, delete_vec, aligned_dim, L_mem, md);
    std::cout << "_________________________Over_deletion_kernel________________"
                 "_________"
              << std::endl;

    // currentFileName = truthset_file + "_without_delete_vec";
    // sync_search_kernel(query, query_num, query_aligned_dim, recall_at,
    // Lsearch,
    //                    sync_index, currentFileName, inactive_tags, res,
    //                    false, true);
    ...
    std::cout << "Search after update, current vector number: " << inMmeorySize
              << " " << start_step << std::endl;

    if (inMmeorySize >= start_step) {
      get_io_info("begin");
      std::cout << "Begin Merge" << std::endl;
      merge_kernel<T, TagT>(sync_index, save_path);
      get_io_info("end");
      std::cout << "IO_总" << std::endl;
      inMmeorySize = 0;
    }
  ...
  //       std::async(std::launch::async, insertion_kernel<T, TagT>,
  //       data_load,
  //                  std::ref(sync_index), std::ref(insert_vec),
  //                  aligned_dim);
  ...
  //       do {
  //         merge_status = merge_future.wait_for(std::chrono::seconds(5));
  //       } while (merge_status != std::future_status::ready);
  //       if ((i + 1) % 10 == 0)
  //         sync_search_kernel(query, query_num, query_aligned_dim,
  //         recall_at,
  //                            Lsearch, sync_index, truthset_file,
  //                            inactive_tags, base_num, false, true);
  //     }
...
void build(const std::string& data_path, const unsigned L_mem,
  ...
  // diskann::SyncIndex<T, TagT> sync_index(num_points + 5000, dim,
  // num_shards,
  //                                        paras, 2, save_path);
...
int main(int argc, char** argv) {
  ...
  int         md = (int) std::stoi(argv[arg_no++]);
  int         start_step = (int) std::atoi(argv[arg_no++]);

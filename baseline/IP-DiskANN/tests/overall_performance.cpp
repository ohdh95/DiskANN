...
#pragma omp parallel for num_threads(NUM_SEARCH_THREADS)
  ...
  if (calRecall) {
    ...
    //   for (size_t j = 0; j < gt_dim; j++)
    //     std::cout << query_result_tags[i * gt_dim + j] << " ";
    //   std::cout << std::endl;
    // }
...
void get_trace(tsl::robin_set<uint32_t>& inactive_set,
  ...
  // std::cout << "insert_tags: ";
  // for (int i = 0; i < update_size; i++) {
  //   std::cout << insert_tags[i] << " ";
  // }
  // std::cout << "\n";
  // std::cout << "delete_tags: ";
  // for (int i = 0; i < update_size; i++) {
  //   std::cout << delete_tags[i] << " ";
  // }
  // std::cout << "\n";
  delete[] data_load;
...
void update(const std::string& data_path, const unsigned L_mem,
            ...
            uint32_t id_map = 0, int m = -1, uint32_t mt = 1000) {
  ...
  for (int i = 0; i < batch; i++) {
    ...
    deletion_kernel(data_load, sync_index, delete_vec, aligned_dim, L_mem, m);
    ...
    // Lsearch,
    //                    sync_index, truthset_file, inactive_tags, base_num,
    //                    false, true);
    if (inMmeorySize >= mt) {
      get_io_info("begin");
    ...
    // std::string indir = save_path + "_disk.index";
    // std::string outdir = indir + +"_" + std::to_string(i);
    // copy_file(indir, outdir);
    // if (i != 0) {
    //   // 删除
    //   std::string command = "rm " + indir + "_" + std::to_string(i - 1);
...
int main(int argc, char** argv) {
  ...
  int      m = std::atoi(argv[25]);
  uint32_t mt = std::atoi(argv[26]);
